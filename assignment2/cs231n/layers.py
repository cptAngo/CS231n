...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = {'x':x, 'mu':sample_mean, 'sigma':sample_var, 'gamma':gamma, 'beta':beta, 'norm_x':norm_x, 'eps':eps}
        ...
        #######################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        nm_x = (x - running_mean)/np.sqrt(running_var + eps)
        out = nm_x*gamma + beta
...
def batchnorm_backward(dout, cache):
    ...
    N = cache['x'].shape[0]
    dx = (1./N)*cache['gamma']*((np.sqrt(cache['sigma'] + cache['eps']))**(-1))*(-dgamma*cache['norm_x'] + N*dout - np.ones((N,1)) * dbeta)
